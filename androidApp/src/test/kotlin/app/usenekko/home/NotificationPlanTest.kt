...
import app.usenekko.home.domain.buildDayPlans
import app.usenekko.home.domain.buildDuePlan
import app.usenekko.home.domain.nextOccurrence
import app.usenekko.shared.notifications.MaxScheduledDays
...
class NotificationPlanTest {
    ...
    fun digestDecisionTable() {
        ...
        val scheduled = app.usenekko.shared.notifications.decideDigestSchedule(
            dayKey, fireAt, earlyMorning, quietCutoff, deliveredDays = emptySet(),
            source = ReconcileSource.FOREGROUND,
        )
        assertEquals(
            app.usenekko.shared.notifications.DigestDecision.Schedule(fireAt, isCatchUp = false),
            scheduled,
        )

        val delivered = app.usenekko.shared.notifications.decideDigestSchedule(
            dayKey, fireAt, now, quietCutoff, deliveredDays = setOf(dayKey),
            source = ReconcileSource.BACKGROUND,
        )
        assertEquals(app.usenekko.shared.notifications.DigestDecision.SkipDelivered, delivered)

        val surface = app.usenekko.shared.notifications.decideDigestSchedule(
            dayKey, fireAt, afternoon, quietCutoff,
            deliveredDays = emptySet(), source = ReconcileSource.FOREGROUND,
        )
        assertEquals(app.usenekko.shared.notifications.DigestDecision.SurfaceInApp, surface)

        val catchUp = app.usenekko.shared.notifications.decideDigestSchedule(
            dayKey, fireAt, afternoon, quietCutoff,
            deliveredDays = emptySet(), source = ReconcileSource.BACKGROUND,
        )
        assertTrue(catchUp is app.usenekko.shared.notifications.DigestDecision.Schedule)
        catchUp as app.usenekko.shared.notifications.DigestDecision.Schedule
        assertTrue(catchUp.isCatchUp)
        assertEquals(afternoon + 120_000L, catchUp.fireAtEpochMillis)

        val deferred = app.usenekko.shared.notifications.decideDigestSchedule(
            dayKey, fireAt, lateNight, quietCutoff,
            deliveredDays = emptySet(), source = ReconcileSource.BACKGROUND,
        )
        assertEquals(app.usenekko.shared.notifications.DigestDecision.DeferToTomorrow, deferred)
    }
