...
class AccountRepositoryTest {
    ...
    fun warmLoadReturnsCachedProfileAndBadgesWithoutReads() = runTest {
        ...
        assertEquals("Jane Bell", (result as app.usenekko.shared.domain.Result.Success).data.profile?.resolvedName)
    ...
    fun staleLoadReturnsOldSnapshotThenPublishesFreshProfile() = runTest {
        ...
        assertEquals("Jane Bell", (cached as app.usenekko.shared.domain.Result.Success).data.profile?.resolvedName)
    ...
    fun failedRefreshRetainsUsableSnapshot() = runTest {
        ...
        contactDataSource.badgesError = app.usenekko.home.domain.ContactError.Network
        repository.invalidate()
        val result = repository.load(forceRefresh = true)

        assertTrue(result is app.usenekko.shared.domain.Result.Error)
    ...
    fun changingAccountNeverReturnsPreviousSnapshot() = runTest {
        ...
        assertEquals("Second User", (result as app.usenekko.shared.domain.Result.Success).data.profile?.resolvedName)
