...
-- Persist overdue scheduled occurrences separately from the mutable contact
-- schedule. A missed row is immutable history and is unique per contact/date.
...
-- Time passing cannot fire a database trigger, so Home calls this idempotent
-- RPC before reading the calendar. It materializes every overdue occurrence.
